...
const AuthProvider = (param)=>{
    ...
    const login = async (email, password)=>{
        try {
            const res = await fetch("".concat(apiBase, "/auth/login"), {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    const logout = async ()=>{
        try {
            await fetch("".concat(apiBase, "/auth/logout"), {
                method: 'POST',
                credentials: 'include'
            });
        } catch (error) {
    ...
    const isAdmin = ()=>{
        var _user_user;
        return user && (user.role === 'admin' || ((_user_user = user.user) === null || _user_user === void 0 ? void 0 : _user_user.role) === 'admin');
    };
...
//# sourceMappingURL=_8bb5ad4d._.js.map