...
function EditCouponContent() {
  ...
  useEffect(() => {
    // Fetch categories
    fetch("https://vibe-bites-backend.onrender.com/api/products/categories")
      .then(res => res.json())
      .then(data => setCategories(data.data.categories || []));
    // Fetch coupon details
    if (couponId) {
      fetch(`https://vibe-bites-backend.onrender.com/api/admin/coupons/${couponId}`, {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      const res = await fetch(`https://vibe-bites-backend.onrender.com/api/coupons/${couponId}`, {
        method: "PUT",
        headers: getAuthHeaders(),
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <form onSubmit={handleSubmit} className="max-w-lg mx-auto bg-white p-6 rounded-lg shadow">
          ...
          <div className="mb-4">
            ...
            <input type="number" name="maxDiscount" value={form.maxDiscount} onChange={handleChange} className="border px-3 py-2 rounded" min="0" />
          </div>
          <div className="mb-4">
          ...
          <div className="mb-6 flex justify-center">
            <button type="submit" className="bg-primary text-white px-8 py-3 rounded-lg text-lg font-semibold shadow hover:bg-primary-dark transition border-2 border-primary" disabled={loading} style={{ minWidth: "180px" }}>
